...
- [`hg_pagerank()`](https://mohsaqr.github.io/hypernets/reference/hg_pagerank.md)
  stays as the tidy verb (personalization, sparse path, convergence
  warning) beside `hypergraph_centrality(type = "pagerank")`; parity to
  `1e-10` is tested rather than merging the two engines.
...
- **The merge drops the `Nestimate` dependency.** texthypergraph imports
  ...
  **cannot** be submitted; merged, the dependency disappears and the
  blocker with it.
- The families are loosely coupled: memory and simplicial call nothing
  in the hypergraph family, and the only cross-family call is
  ...
  Four families coexist with file prefixes, no tangle.
...
set, ours, since they are the tidier `as.data.frame(x, what =)` versions
written under Rule 0.
...
- `benchmarks/` (R8/R52/MR/Ohsumed/20NG harness) and
  ...
  `.Rbuildignore`.
- `local_testing_and_equivalence/`: merge both, keeping every oracle.
  The Nestimate identity tests stay meaningful as a regression guard.
- `_pkgdown.yml`: reference index gains the text and projection
  sections; pkgdown CI fails on unindexed topics, so do this in the same
  commit.
...
6.  Update `CLAUDE.md`, `ROADMAP.md`, `TODO.md`; retire texthypergraph’s
    ...
    only then consider releasing `hypernet_retired`.

## Risks

- **Silent regression via the transduction arg** — the single most
  likely way to lose real capability. Guard it with the R8 benchmark
  before and after.
- **Check time**: ~14,300 R lines, ~2,484 tests, torch and sbert in
  Suggests. `--as-cran` will be slow; keep heavy paths behind
  `skip_on_cran()`.
- **One release cadence** for four unrelated families is the accepted
