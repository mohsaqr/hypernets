...
test_that("hg_compare_communities tabulates sizes, similarity and summaries", {
  ...
  expect_identical(summary(cmp), s)
  expect_output(print(cmp), "2 representations")
  expect_s3_class(plot(cmp), "ggplot")
  expect_s3_class(plot(cmp, what = "similarity"), "ggplot")
  ...
  expect_identical(hg_compare_communities(list(mh = mh, bh = bh))$summary, cmp$summary)
