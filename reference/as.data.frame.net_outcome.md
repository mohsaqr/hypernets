...
as.data.frame(
  ...
  optional = FALSE,
  ...,
  what = c("coefficients", "terms", "features", "dropped"),
  significant = FALSE,
  alpha = 0.05,
  sort_by = NULL,
  top = NULL
...
- what:
  ...
  `"coefficients"` (default) for one row per feature, `"terms"` for the
  same table with the intercept, `"features"` for the per-actor feature
  table on its raw scale, or `"dropped"` for the actors excluded from
  the fit and why.

- significant:

  Logical. For `"coefficients"` and `"terms"`, keep only the rows with
  `p_adj < alpha`. Default `FALSE`.
...
- sort_by:
  ...
  `NULL` (default, model order), or one of `"estimate"` (largest
  absolute estimate first), `"p"` or `"p_adj"` (smallest first).

- top:

  Integer or `NULL`. Return only the first `top` rows, applied after
  every filter and after `sort_by`.

## Value

A base data.frame. For `"coefficients"` and `"terms"`, one row per model
term with `feature`, `estimate`, `std_error`, `conf_low`, `conf_high`,
`statistic`, `p`, `p_adj` (the multiplicity-corrected p-value; `NA` for
the intercept, which is not one of the tested features) and `n` (actors
in the fit). For `"features"`, one row per actor with the key column,
one column per feature on its RAW (un-standardized) scale, `outcome`,
the `cluster` when one was given, and the `n_events` / `n_unmatched`
support counts. For `"dropped"`, one row per excluded actor with the key
column and `reason`.
