...
- metrics:
  ...
  Character vector, any subset of `"persistence"`, `"stationary_prob"`,
  `"return_time"`, `"sojourn_time"`, `"avg_time_to_others"`,
  `"avg_time_from_others"`. Default: all six.

- top:

  Integer or `NULL`. Plot only the `top` states with the largest
...
A `ggplot` object, faceted by metric.
