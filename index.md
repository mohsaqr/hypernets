...
reference — and the Legal hypergraphs paper workflow is
`docs/legal-hypergraphs.html`. Start with **`docs/index.html`** for the
map, or **`docs/overview.html`** to see the same dataset read all three
ways. Rebuild them with `Rscript docs/_knit_all.R`. Longer tutorials
live in `Tutorial_docs/`.
